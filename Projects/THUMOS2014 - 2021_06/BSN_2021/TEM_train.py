...
def binary_logistic_loss(scores,anchors):
    ...
    pmask=tf.clip_by_value(pmask, clip_value_min=0.0000001, clip_value_max=1.0)
    num_positive=tf.reduce_sum(pmask)
    # num_positive = tf.clip_by_value(pmask, clip_value_min=0.0000001, clip_value_max=1.0)
    ...
    # num_sample=[tf.clip_by_value(pmask, clip_value_min=0.0000001, clip_value_max=1.0),ratio]
    return loss,num_sample

def action_binary_logistic_loss(scores,anchors):
    pmask=tf.cast(scores>0.9,dtype=tf.float32)
    pmask=tf.clip_by_value(pmask, clip_value_min=0.0000001, clip_value_max=1.0)
    num_positive=tf.reduce_sum(pmask)
    # num_positive = tf.clip_by_value(pmask, clip_value_min=0.0000001, clip_value_max=1.0)
...
def tem_loss(anchors_action,anchors_start,anchors_end,
    ...
    loss_action,num_sample_action = action_binary_logistic_loss(match_scores_action,anchors_action)
...
class Config(object):
    ...
    def __init__(self):
        #common information
        self.learning_rates=[0.0001]*5 + [0.00001]*15
...
if __name__ == "__main__":
    ...
    axs={"loss_action":ax1,"loss_start":ax2,"loss_end":ax3,"l2":ax4}
